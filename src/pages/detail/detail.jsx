...
export default function DetailProduct(){
    ...
    let onGetData = async() => {
        try {
            let response = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/products/${productId.id}`)
    ...
    let onAddOrder = async() => {
        try {
            ...
            let checkExist = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/cart?idProduct=${data.id}`)

            if(checkExist.data.length === 0){
                let response = await axios.post('http://my-json-server.typicode.com/bachtrw/jcwd2302/cart', dataToSend)
            }else{
                let newQuantity = checkExist.data[0].quantity + 1
                let update = await axios.patch(`http://my-json-server.typicode.com/bachtrw/jcwd2302/cart/${checkExist.data[0].id}`, {quantity: newQuantity})
                console.log(update)
            }   
        } catch (error) {
