...
export default function Register(props){
    ...
    let onSubmit = async() => {
        try {
            ...
            let checkEmail = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/users?email=${inputEmail}`)
            let checkUsername = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/users?username=${inputUsername}`)
            
            if(checkEmail.data.length === 0 && checkUsername.data.length === 0){
                // Post
                await axios.post('http://my-json-server.typicode.com/bachtrw/jcwd2302/users', {username: inputUsername, email: inputEmail, password: inputPassword})
