...
export default function App(){
  ...
  let checkIsLogin = async() => {
    try {
      ...
      if(getTokenId){
        let response = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/users?id=${getTokenId}`)
  ...
  let onLogin = async(inputUsername, inputPassword) => {
      try {
          ...
          let response = await axios.get(`http://my-json-server.typicode.com/bachtrw/jcwd2302/users?username=${inputUsername}&password=${inputPassword}`)
